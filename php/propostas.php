...
function Gravar() {
    ...
    if ($_POST['idOrcamentoComp'] != '' && count((array)$_POST['idOrcamentoComp']) > 0) {
        ...
        $arrayObras = array($_POST['idOrcamentoComp'], 
            $_POST['idsTiposObras'], 
            $_POST['idsObras'], 
            $_POST['idsArtistas'], 
            $_POST['idsTamanhos'], 
        ...
        $sql ="SELECT aob.altura, aob.largura, aob.tiragemMaxima, aob.tiragemAtual, idObra, 
              ...
              (SELECT SUM(tiragemAtual) 
                ...
                WHERE idArtistaObraTamanho= ". $_POST['idArtistaObraTamanho'] ." AND del=0) AS qtdTotalVendida,
                 ...
                 IFNULL((SELECT SUM(qtdVendidoAtual) FROM vendas_comp   WHERE idTamanho=". $_POST['idArtistaObraTamanho']."   AND del=0),0) AS estrelaAtual,
                 IFNULL((SELECT SUM(qtdVendidoAtual) FROM orcamentos_comp  WHERE idTamanho=". $_POST['idArtistaObraTamanho']."   AND del=0),0) AS estrelaAtualOrcamento
...
function getDetalhesAcabamento() {
    ...
    if ($_POST['item'] == 'p') {
        ...
        $sql = "SELECT aob.altura, aob.largura, aob.tiragemMaxima, aob.tiragemAtual, 
            (SELECT SUM(tiragemAtual) 
                FROM artistas_obras_tamanhos 
                WHERE idArtistaObraTamanho= ". $_POST['idObraTamanho'] . " AND del=0 ) AS qtdTotalVendida
...
function GerarPdfOrcamentoImg() {
 ...
 $sql = "SELECT c.cliente, o.telefoneCliente, o.emailCliente, c.responsavel, tt.tipoTransporte, 
    ...
    file_put_contents( '../orcamentos/orcamento-' . $_POST['idOrcamento'] . '-' . $cliente . '.pdf', $output);
...
function AtualizaEstrela(){
    ...
    */
    ...
    if ( $db->n_rows <= 0 ) {
        ...
        $estrelas = intval(($linha['tiragemAtual'] + $_POST['qtd']) / 10);
        
        $json = $db->fetch();
        $json['estrelas'] = $estrelas;
        $json['estrelaAtual'] = $json['estrelaAtual'];
        $json['estrelaAtualOrcamento'] = $json['estrelaAtualOrcamento'];
...
<body style="margin:0px; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
    ...
    <div id="quadro1" style="margin:3px; border:2px solid #000; min-height:950px; width:730px; padding-left:05px; padding-right:05px;">
        <div id="linha1">
            <div style="overflow:hidden; color:' . $corPdf . '; vertical-align:top; display:inline-block; height:80px; width:49.5%; text-align:left; font-weight:bold; font-size:32px; padding-top:05px;">Orçamento</div>
            <div style="float:right; overflow:hidden; color:#000; vertical-align:top; display:inline-block; height:80px; width:49.5%; text-align:right; font-weight:bold; font-size:20px; margin-top:05px;">Nº Orçamento:
                <span>' . number_format_complete($_POST['idOrcamento'], '0', 5) . '</span>
                <br />
                <span>' . $loja . '</span>
            </div>
        </div>
        <!--linha1-->
        <div id="linha2" style="background:' . $corPdf . '; padding:4px; margin-bottom:5px;"></div>
        <div style="background:#FFF; width:49.5%; height:125px; display:inline-block; vertical-align:top; float:left;">
        ...
        <div style="background:#FFF; width:49.5%; height:125px; display:inline-block; vertical-align:top; float:right;">

            <div style="width:70px; font-size:11px; font-weight:bold; line-height:22px; vertical-align:top; text-align:left; width:auto;">Emissão: ' . $dataOrcamento . ' </div>

            <img src="' . $logoPdf . '" style="' . $medidasLogo . '" />

            <div style="width:70px; font-size:11px; font-weight:bold; line-height:22px; vertical-align:top; text-align:left; width:auto;">Valido até: ' . $dataValidade . ' </div>
        ...
        foreach ($linhas as $linha){ 
            ...
            if($linha['idTipoProduto'] == '1'){
                ...
                $html .= '<tr>
                            ...
                            <td>' . ListarTamanhosObras($db, $linha['idObra'], $_POST['idOrcamento'], $linha['idsInstaarts']) . '</td>
                        </tr>';
    ...
    $html1 .= '<div id="quadro2" style="margin:0px; border:1px solid #000; min-height:1024px; width:730px; padding-left:05px; padding-right:05px;">';  
...
function ListarTamanhosObras($ArqT, $idObra, $idOrcamento, $idsInstaarts){
    ...
    if ( $ArqT->n_rows <= 0 ) {
        ...
        while ($linha = $ArqT->fetch()) {  
            ...
            if($tamanhos == ''){
                $tamanhos = $linha['nomeTamanho'] . ' (' . 
                        FormatMoeda($linha['altura']) . 'x' . 
                        FormatMoeda($linha['largura']) . ')' . ' - R$' . 
                        FormatMoeda($linha['valor']);
            }else{
                $tamanhos .= '<br/>' . $linha['nomeTamanho'] . ' (' . 
                        FormatMoeda($linha['altura']) . 'x' . 
                        FormatMoeda($linha['largura']) . ')' . ' - R$' . 
                        FormatMoeda($linha['valor']);
            }
...
function CalcularFrete(){
    ...
    //$sql = "SELECT idloja FROM loja WHERE idloja = " . $_POST['sCepOrigem'];
        
    //$db->query( $sql );

    //$result = $db->fetch();
    
    //$sCepOrigem = $result['cep'];
    ...
    for ($i = 0; $i < $x; $i++) {
        ...
        $xSelAux = count($selecionadoAux);
        
        for ($j = 0; $j < $xSelAux; $j++) {

            if($selecionadoAux[$j] == '1' && $alturaAux[$z] != '00' && $larguraAux[$z] != '00'){
                ...
                $xmlCorreios = "http://ws.correios.com.br/calculador/CalcPrecoPrazo.asmx/CalcPrecoPrazo?nCdEmpresa=" . $_POST['nCdEmpresa'] . 
                        ...
                        "&nVlAltura=" . $alturaAux[$z] . 
                        "&nVlLargura=" . $larguraAux[$z] . 
