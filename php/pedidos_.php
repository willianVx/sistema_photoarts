...
function Gravar() {
    ...
    $sql = "vendas SET dataVenda = '" . DataSSql($_POST['dataCadastro']) . "', 
            ...
            "obsContrato='" . $db->escapesql($ArqT, $_POST['obsContrato']) . "', " .
    ...
    if ( $db->n_rows <= 0) {
        ...
        //GRAVA OS PAGAMENTOS
        GravarParcelas($db, ($_POST['codigo'] > 0 ? $_POST['codigo'] : $idPedido));

        if ($_POST['codigo'] <= 0) {
            GerarOrdemProducaoAutomatica($$db, $idPedido, $_POST['idLoja'], $_POST['idsVendasObras'], $arrayObras);
        }
        echo $idPedido;
    }
...
function CalculaPrevisaoEntrega() {
    ...
    /* $dataAtual = DataSSql($_POST['dataAtual'], false);
      ...
      $codigoDia = mysqli_result($Tb, '0', "codigoDia");
...
function EnviarPdfPedidoEmail() {
    ...
    $remetenteEmail = ($result2["emailCorporativo"] == '' ? $_SESSION['photoarts_email'] : $result2["emailCorporativo"]); //($idTipoProduto == '1' ? 'atendimento@photoarts.com.br' : 'contato@instaarts.com.br');

    //$responderPara = $_SESSION['photoarts_email'];//mysqli_result($Tb2, 0, "email");
    $responderPara = ($result2["emailCorporativo"] == '' ? $remetenteEmail : $result2["emailCorporativo"]);
