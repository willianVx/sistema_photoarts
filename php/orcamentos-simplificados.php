    ...
    if ( $db->n_rows <= 0) {
        ...
        foreach ($linhas as $linha){        
            ...
            $json[] = array(
                ...
                'obras' => getObras($db, $linha['idOrcamentoSimplificado'], ($linha['idTipoProduto']==1?'1':'0')),
...
function Mostrar() {
    ...
    if ( $db->n_rows <= 0) {
        ...
        $json = array(
            ...
            'valorDesconto' => '0,00',
            'valorTotal' => '0,00',
            'arrayObras' => ListarObras($db, $linha['idOrcamentoSimplificado'])
        );
...
function MostrarObras() {
    ...
    if ($_POST['idOrcamentoSimplificado'] <= '0' || $_POST['salvas'] == '0') {
        ...
        $sql = "SELECT IF(IFNULL(p.nomeProduto, '') = '', idObra, osc.idProduto) AS idArtistaObra, 
                IF(IFNULL(p.nomeProduto, '') = '', IFNULL(ao.nomeObra, 'InstaArts'), p.nomeProduto) AS nomeObra, 
                osc.imagemObra AS imagem, 
                IF(IFNULL(p.nomeProduto, '') = '', IFNULL(a.artista, 'Instaarts'), '- - -') AS nomeArtista, 
                IFNULL(ac.nomeAcabamento, '- - -') AS nomeAcabamento, IFNULL(ao.idArtista, 0) AS idArtista, 
                osc.idAcabamento, GROUP_CONCAT(idOrcamentoCompInstaarts) AS idsInstaarts, osc.idTipoProduto, 
                osc.idMolduraGrupo, osc.idMoldura, IFNULL(m.moldura, '') AS moldura, 
                IFNULL(m.imagem, '') AS imagemMoldura, osc.qtd, osc.valor, osc.idOrcamentoComp
                ...
                WHERE osc.idOrcamento = " . $_POST['idOrcamentoSimplificado'] . " AND osc.del = 0 ";
        ...
        //$sql .= " ORDER BY osc.idOrcamentoSimplificado, nomeArtista, nomeObra ";
                $sql .= "GROUP BY IF(osc.idProduto > 0, osc.idOrcamentoComp, osc.imagemObra), idOrcamentoCompInstaarts 
                ORDER BY osc.idTipoProduto ASC, osc.idObra";
    ...
    if ( $db->n_rows <= 0) {
        ...
        foreach ($linhas as $linha){
            ...
            $json[] = array(
                ...
                'idMoldura' => $linha['idMoldura'],
                'moldura' => $linha['moldura'],
                'imagemMoldura' => $linha['imagemMoldura'],
                'valor' => $linha['valor'],
                'qtd' => $linha['qtd'],
                'quantidadeValor' => 'Quantidade: ' . $linha['qtd'] . ' - Valor: ' . FormatMoeda($linha['valor']),
                'idOrcamentoComp' => $linha['idOrcamentoComp'], 
                'tamanhos' => MostrarTamanhosObras($db, $linha['idArtistaObra'], $_POST['idAcabamento'], $_POST['idOrcamentoSimplificado'], ($idArtista==-2 ? $linha['idArtista'] : $idArtista), $linha['idsInstaarts'])
            );
        }
...
function MostrarTamanhosObras($ArqT, $idObra, $idAcabamento, $idOrcamentoSimplificado, $idArtista, $idsInstaarts) {
    ...
    if ($idOrcamentoSimplificado <= 0 || $_POST['salvas'] == '0') {
        ...
        $sql = "SELECT precoBase, pesoBase, indiceAte1MSemEstrela, indiceAte1MComEstrela, indiceAcima1MSemEstrela, 
                indiceAcima1MComEstrela, valorAcresMoldura 
        ...
        if ($_POST['item'] == '1') {
            ...
            $sql = "SELECT aot.idArtistaObraTamanho, t.idTamanho, t.nomeTamanho, aot.altura, aot.largura, aot.tiragemMaxima,           
                aot.tiragemAtual, 0 AS selecionado, 0 AS idOrcamentoSimplificadoComp,               
                (SELECT SUM(tiragemAtual) 
                FROM artistas_obras_tamanhos 
                WHERE idArtistaObraTamanho=aot.idArtistaObraTamanho AND del=0) AS qtdTotalVendida
            ...
            if ( $ArqT->n_rows <= 0) {
                ...
                while ($linha = $ArqT->fetch()) {
                    ...
                    $altura = ($_POST['altura'] != ',' && $_POST['altura'] != '' ? (floatval(ValorE($_POST['altura'])) + floatval($linha['altura'])) : $linha['altura']);
                    $largura = ($_POST['largura'] != ',' && $_POST['largura'] != '' ? (floatval(ValorE($_POST['largura'])) + floatval($linha['largura'])) : $linha['largura']);
                    ...
                    $qtdestrelas = $qtdestrelas > 5 ? 5 : $qtdestrelas;
                    ...
                    $json[] = array(
                        ...
                        'altura' => FormatMoeda($altura),//FormatMoeda($linha['altura']),
                        'largura' => FormatMoeda($largura),//FormatMoeda($linha['largura']),
                        'valor' => FormatMoeda($valorObra),
                        'selecionado' => $linha['selecionado'],
                        'idTamanho' => $linha['idTamanho'],
                        'estrelas' => $qtdestrelas,
                        'qtdTotalVendida' => $linha['qtdTotalVendida'],
                        'pesoObra' => $pesoObra
                    );
                }
        ...
        if($idObra == '0'){
            ...
            $sql = "SELECT oc.idOrcamentoComp, oc.idTamanho, t.nomeTamanho, oc.altura, oc.largura, oc.valor, 
                    ...
                    WHERE oc.del=0 AND oc.idOrcamento = " . $idOrcamentoSimplificado . " AND oc.idTipoProduto = 2 
                    AND idOrcamentoCompInstaarts IN(" . $idsInstaarts . ") 
            ...
            $sql = "SELECT osc.idOrcamentoComp, osc.idTamanho, t.nomeTamanho, osc.altura, osc.largura, 
                    osc.valor, osc.selecionado, osc.estrelasAtual AS estrelas, osc.qtdVendidoAtual, pesoObra
                    FROM orcamentos_comp AS osc 
                    INNER JOIN artistas_obras_tamanhos AS aot ON aot.idArtistaObraTamanho = osc.idTamanho
                    INNER JOIN tamanhos AS t ON t.idTamanho = aot.idTamanho 
                    WHERE osc.del=0 AND osc.idObra = " . $idObra . " AND osc.idOrcamento = " . $idOrcamentoSimplificado;
        ...
        if ( $ArqT->n_rows <= 0) {
            ...
            while ($linha = $ArqT->fetch()) {
                ...
                $json[] = array(
                    'idOrcamentoSimplificadoComp' => $linha['idOrcamentoComp'],
                    ...
                    'qtdVendidoAtual' => $linha['qtdVendidoAtual'],
                    'pesoObra' => $linha['pesoObra']
                );
            }
...
//border:1px solid #000; height:1200px; 
    ...
    file_put_contents( '../orcamentos/orcamento-simplificado-' . $_POST['idOrcamento'] . '-' . $cliente . '.pdf', $output);
    
    $random = rand();

    $filepath = 'orcamentos/orcamento-simplificado-' . $_POST['idOrcamento'] . '-' . $cliente . '.pdf';
