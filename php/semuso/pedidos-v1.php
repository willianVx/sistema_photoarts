...
function GerarPdfPedido() {
    ...
    if (file_exists('../pedidos/pedido-' . $_POST['idPedido'] . '.pdf')) {
        unlink('../pedidos/pedido-' . $_POST['idPedido'] . '.pdf');
    }
    ...
    $filepath = 'pedidos/pedido-' . $_POST['idPedido'] . '.pdf?PID=' . rand();
...
function AvisoRetirada() {
    ...
    $sql = "SELECT idTipoEntrega, tr.tipotransporte, c.apelido, c.email, envioPesquisa FROM vendas AS v
    ...
    if (EnvioDeEmailsPhotoarts($apelido, $email, $remetenteEmail, $remetente, '', '', $remetenteEmail, $remetente, $assunto, $mensagem, '', $idTipoProduto)) {
        echo '1';
    } else {
        echo '0';
    }
...
function AvisoPesquisa($idPedido) {
   ...
   // $sql = "SELECT idTipoProduto FROM orcamentos_comp WHERE idOrcamento = " . $_POST['idPedido'] . " AND del = 0 LIMIT 1";
    ...
    $sqll  = "SELECT idTipoEntrega, tr.tipotransporte, c.apelido, c.email , envioPesquisa, idUltimoStatus FROM vendas AS v
        INNER JOIN transportes_tipos AS tr ON tr.idTransporteTipo = v.idTipoEntrega
        INNER JOIN clientes AS c USING (idCliente)
        WHERE idVenda= " . $idPedido;
