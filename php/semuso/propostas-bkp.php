...
function Gravar() {
    ...
    //GRAVA AS OBRAS
    if ($_POST['idOrcamentoComp'] != '' && count($_POST['idOrcamentoComp']) > 0) {
...
function GerarPdfOrcamento2() {
    ...
    $sql = "SELECT osc.idObra, IFNULL(ao.nomeObra, 'InstaArts') AS nomeObra, osc.imagemObra, 
            ...
            GROUP BY osc.imagemObra, osc.idOrcamentoComp
            ORDER BY osc.idObra";
        ...
        while ($linha = mysql_fetch_assoc($Tb)) {
            ...
            if($linha['idTipoProduto'] == '1'){
                ...
                $html .= '<tr>
                            ...
                            <td>Artista: ' . $linha['artista'] . '<br/> Obra: ' . $linha['nomeObra'] . '<br/> Acabamento: ' . $linha['nomeAcabamento'] . ' <br/> Moldura: ' . $linha['moldura'] . '</td>
                            <td>' . ListarTamanhosObras($ArqT, $linha['idObra'], $_POST['idOrcamento'], $linha['idsInstaarts']) . '</td>
                        </tr>';
            }else if($linha['idTipoProduto'] == '2'){

                $html .= '<tr>
                            <td align="center"><img src="' . ($linha['imagemObra'] == 'semimagem.png' ? '../imagens/semimagem.png' : '../imagens/instaarts/' . $linha['imagemObra']) . '" style="width:100px; height:auto;"/></td>
                            <td>Instaarts<br/> Acabamento: ' . $linha['nomeAcabamento'] . ' <br/> Moldura: ' . $linha['moldura'] . '</td>
                            <td>' . ListarTamanhosObras($ArqT, $linha['idObra'], $_POST['idOrcamento'], $linha['idsInstaarts']) . '</td>
                        </tr>';
            }else{
    ...
    $random = rand();
    $pdf->Output('../orcamentos/orcamento-' . $_POST['idOrcamento'] . '.pdf', 'F');
    $filepath = 'orcamentos/orcamento-' . $_POST['idOrcamento'] . '.pdf?' . $random;
...
function ListarTamanhosObras($ArqT, $idObra, $idOrcamento, $idsInstaarts){
    ...
    if($idObra == '0'){
        ...
        $sql = "SELECT oc.idOrcamentoComp, oc.idTamanho, t.nomeTamanho, oc.altura, oc.largura, oc.valor 
                ...
                WHERE oc.idOrcamento = " . $idOrcamento . " AND oc.idTipoProduto = 2 
        ...
        $sql = "SELECT osc.idOrcamentoComp, osc.idTamanho, t.nomeTamanho, osc.altura, osc.largura, 
                ...
                WHERE osc.idObra = " . $idObra . " AND osc.idOrcamento = " . $idOrcamento . " AND osc.selecionado = 1";
