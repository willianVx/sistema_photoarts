...
function Pesquisar(){
    ...
    $sql = "SELECT l.loja, v.idVenda, vc.idVendaComp, vp.idVendaParcela, LPAD(v.idVenda, 5, '0') AS numeroPedido, v.dataVenda, 
            ...
            INNER JOIN vendedores AS ve ON ve.idVendedor = v.idVendedor AND ve.gerente = 1 
    ...
    if($_POST['situacao'] == '1'){
        ...
        //$sql .= " AND IFNULL(vr.idVendedorRepasse, 0) NOT IN(SELECT IFNULL(GROUP_CONCAT(idMarchandGerenteRepasse), 0) FROM vendas_parcelas_repasses WHERE idMarchandGerenteRepasse <> 0)";
    }else{
        $sql .= " AND vp.idVendaParcela IN(SELECT IFNULL(idVendaParcela, 0) FROM vendas_parcelas_repasses)";
        //$sql .= " AND IFNULL(vr.idVendedorRepasse, 0) IN(SELECT IFNULL(GROUP_CONCAT(idMarchandGerenteRepasse), 0) FROM vendas_parcelas_repasses WHERE idMarchandGerenteRepasse <> 0)";
    }
...
function FecharRepasses(){
    ...
    $sql = "INSERT INTO conpag SET 
            ...
            descricao = 'FECHAMENTO DO REPASSE DE MARCHANDS GERENTES NO PERÍODO DE " . $_POST['de'] . " ATÉ " . $_POST['ate'] . "', 
    ...
    for($i = 0; $i < count($idVenda); $i++){
        ...
        /*$sql = "UPDATE vendas_comp SET 
                idVendedorGerenteRepasse = " . UltimoRegistroInserido($ArqT) . " 
                WHERE idVendaComp = " . $idVendaComp[$i];*/

        $sql = "INSERT INTO vendas_parcelas_repasses SET 
                idVendaParcela = " . $idVendaParcela[$i] . ", 
                idMarchandGerenteRepasse = " . UltimoRegistroInserido($db);
...
function ExportarPlanilha() {
    ...
    if($_POST['situacao'] == '1'){
        $sql .= " AND vp.idVendaParcela NOT IN(SELECT IFNULL(GROUP_CONCAT(idVendaParcela), 0) FROM vendas_parcelas_repasses)";
        //$sql .= " AND IFNULL(vr.idVendedorRepasse, 0) NOT IN(SELECT IFNULL(GROUP_CONCAT(idMarchandGerenteRepasse), 0) FROM vendas_parcelas_repasses WHERE idMarchandGerenteRepasse <> 0)";
    }else{
        $sql .= " AND vp.idVendaParcela IN(SELECT IFNULL(GROUP_CONCAT(idVendaParcela), 0) FROM vendas_parcelas_repasses)";
        //$sql .= " AND IFNULL(vr.idVendedorRepasse, 0) IN(SELECT IFNULL(GROUP_CONCAT(idMarchandGerenteRepasse), 0) FROM vendas_parcelas_repasses WHERE idMarchandGerenteRepasse <> 0)";
    }
    ...
    $newPath = '../relatorio-de-repasses-de-marchands-gerentes-user-' . $num . '.xls';
    $newPathReturn = 'relatorio-de-repasses-de-marchands-gerentes-user-' . $num . '.xls';
