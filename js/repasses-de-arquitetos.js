checkSessao();
CheckPermissao(176, false, '', true);

window.onload = function () {

    Selector.$("legendaTela").innerHTML = "<div>Repasses de Arquitetos</div>";
    carregarmenu();
    getDadosUsuario();
    ...
    getArquitetos(Selector.$('arquiteto'), 'Todos', true);
    Pesquisar();
...
function Pesquisar() {
    ...
    if(Selector.$('arquiteto').value > 0 && Selector.$('situacao').value == '1'){
    ...
    ajax.ajax.onreadystatechange = function () {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            verConpag.setAttribute('onclick', 'window.location="cadastro-de-contas-a-pagar.html?source=repasses-de-arquitetos&idConpag=' + json[i].idConpag + '"');
...
function PromptFecharRepasses(){
    ...
    if(!CheckPermissao(175, true, 'Você não possui permissão para fazer o fechamentos dos repasses', false)){
        return;
    }
...
function Imprimir_onClick() {
    ...
    if(!CheckPermissao(174, true, 'Você não possui permissão para imprimir o repasses de arquitetos', false)){
        return;
    }
    ...
    window.open('impressao-de-relatorios.html?source=repasses-de-arquitetos');
}

function botExportar_onClick() {

    if(!CheckPermissao(173, true, 'Você não possui permissão para gerar o excel dos repasses dos arquitetos', false)){
        return;
    }
    ...
    var ajax = new Ajax('POST', 'php/repasses-de-arquitetos.php', false);
