var codigoAtual = 0;
checkSessao();
CheckPermissao(8, false, '', true);

window.onload = function () {

    Selector.$("legendaTela").innerHTML = "<div>Cadastro de Colecionadores</div>";
    carregarmenu();
    getDadosUsuario();
    ...
    if(!VerificarAdmin()){
        gridColecionadores.hiddenCol(11);
    }
...
function MostraColecionadores(aviso) {
    ...
    ajax.ajax.onreadystatechange = function () {
        ...
        if (ajax.getResponseText().trim() == '0') {
            ...
            Selector.$('msgResult').innerHTML = 'Nenhum colecionador encontrado';
            return;
        }
        ...
        if(json.length > 1){
            Selector.$('msgResult').innerHTML = json.length + ' colecionadores listados';
        }
        else{
            Selector.$('msgResult').innerHTML = '1 colecionador listado';
        }
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            excluir.setAttribute('title', 'Excluir Colecionador');
            excluir.setAttribute('style', 'cursor:pointer');
            excluir.setAttribute('onclick', 'ExcluirColecionadorAux(' + gridColecionadores.getRowCount() + ')');
            ...
            gridColecionadores.getCell(gridColecionadores.getRowCount() - 1, 8).setAttribute('style', 'text-align:left;');
            gridColecionadores.getCell(gridColecionadores.getRowCount() - 1, 9).setAttribute('style', 'text-align:center; width:60px;' + (json[i].premium == 'SIM' ? 'color:green;' : ''));
            
            gridColecionadores.getCell(gridColecionadores.getRowCount() - 1, 10).setAttribute('style', 'text-align:center; width:60px;' + (json[i].arquiteto.substr(0, 3) == 'SIM' ? 'color:green;' : ''));
            gridColecionadores.getCell(gridColecionadores.getRowCount() - 1, 11).setAttribute('style', 'text-align:center; width:40px;');
            gridColecionadores.getCell(gridColecionadores.getRowCount() - 1, 12).setAttribute('style', 'text-align:center; width:40px;');

            pintaLinhaGrid(gridColecionadores);
        }

        if(!VerificarAdmin()){
            gridColecionadores.hiddenCol(12);
        }
    };
    
    Selector.$('msgResult').innerHTML = "Aguarde, pesquisando...";
    ajax.Request(p);
}

function ExcluirColecionadorAux(linha) {

    mensagemExcluirColecionador = new DialogoMensagens("prompt", 125, 380, 150, "4", "Atenção!", "Deseja realmente excluir este colecionador?", "OK", "ExcluirColecionador(" + linha + ");", true, "");
    mensagemExcluirColecionador.Show();
}

function ExcluirColecionador(linha){

    mensagemExcluirColecionador.Close();

    var ajax = new Ajax('POST', 'php/clientes.php', false);
    var p = 'action=ExcluirColecionador';
    p+= '&idCliente=' + gridColecionadores.getRowData(linha);
    ajax.Request(p);

    if(ajax.getResponseText() == '0'){
        var mensagem = new DialogoMensagens("prompt", 140, 400, 150, "1", "Erro!", "Erro ao excluir o colecionador. Se o erro persistir contate o suporte técnico.", "OK", "", false, "");
        mensagem.Show();
        return;
    }else{
        gridColecionadores.deleteRow(linha);
    }
}
...
function Gravar() {
    ...
    if(!CheckPermissao(9, true, 'Você não possui permissão para editar um colecionador', false)){
        return;
    }
...
function Mostra(Codigo, ajustar) {
    ...
    if(!CheckPermissao(163, true, 'Você não possui permissão para Visualizar detalhes sobre o  colecionador', false)){
        return;
    }
    ...
    MostrarFinanceiro(json.arrayFinanceiro);
    ...
    if(!CheckPermissao(51, false, '', false)){
        Selector.$('aba2').style.display = 'none';
        Selector.$('div2').style.display = 'none';
...
function Novo(ajustar) {
    ...
    if(!CheckPermissao(9, true, 'Você não possui permissão para cadastrar um novo colecionador', false)){
        return;
    }
...
function AjustarDivs() {
    ...
    if (Selector.$('divRel').clientHeight == "0") {
        ...
        Selector.$('loja').style.display = 'inline-block';
        Selector.$('msgResult').style.display = 'inline-block';
        ...
        Selector.$('loja').style.display = 'none';
        Selector.$('msgResult').style.display = 'none';
...
function excluirFollow(codigo) {
    ...
    if(!CheckPermissao(156, true, 'Você não possui permissão para excluir um follow up', false)){
        return;
    }
...
function editarFollow(codigo) {
    ...
    if(codigo <= 0){
        ...
        if(!CheckPermissao(10, true, 'Você não possui permissão para cadastrar um novo follow up', false)){
            return;
        }
    }
...
function GravarFollow(codigo) {
    ...
    if(!CheckPermissao(10, true, 'Você não possui permissão para editar um follow up', false)){
        return;
    }
