...
function Mostra() {
    ...
    var ajax = new Ajax('POST', 'php/tipos-de-equipamentos.php', true);
    var p = 'action=Mostra';
    ...
    ajax.ajax.onreadystatechange = function () {
        ...
        if (ajax.getResponseText() === '0') {
            ...
            Selector.$('msgEquip').style.display = "block";
            return;
        }

        Selector.$('divTabela').style.display = "block";
        Selector.$('msgEquip').style.display = "none";
...
function promptCadastro(codigo, nome, ativo) {
    ...
    lblnome.innerHTML = 'Nome do equipamento:';
    ...
    txtnome.setAttribute("style", 'margin-left:10px; width:230px');
...
function Gravar(codigo) {
    ...
    ajax.ajax.onreadystatechange = function () {
        ...
        if (ajax.getResponseText() == '2') {
            Selector.$('ativo').focus();
            var mensagem = new DialogoMensagens("prompt", 120, 350, 150, "2", "Atenção!", "Este tipo de equipamento já está cadastrado.", "OK", "", false, "nome");
            mensagem.Show();
            return;
        } else if (ajax.getResponseText() == '0') {
            Selector.$('ativo').focus();
            var mensagem = new DialogoMensagens("prompt", 140, 350, 150, "1", "Erro!", "Problemas ao gravar o tipo de equipamento. Tente novamente.", "OK", "", false, "");
            mensagem.Show();
            return;
        } else {
