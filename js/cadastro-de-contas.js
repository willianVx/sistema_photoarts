checkSessao();
CheckPermissao(124, false, '', true);

window.onload = function () {

    Selector.$("legendaTela").innerHTML = "<div>Cadastro de Contas</div>";
    carregarmenu();
    getDadosUsuario();
...
function Mostra() {
    ...
    var ajax = new Ajax('POST', 'php/cadastro-de-contas.php', true);
    var p = 'action=Mostra';
...
function promptCadastro(codigo, nome, ativo) {
        ...
        if(!CheckPermissao(125, true, 'Você não possui permissão para cadastrar/editar uma conta', false)){
            return;
        }
...
function Gravar(codigo) {
    ...
    if(!CheckPermissao(125, true, 'Você não possui permissão para editar uma conta', false)){
        return;
    }
    ...
    if (nome.trim() === '') {
        ...
        var mensagem = new DialogoMensagens("prompt", 120, 350, 150, "2", "Atenção!", "Por favor preencha o campo conta.", "OK", "", false, "nome");
        mensagem.Show();
        return;
    }
    ...
    ajax.ajax.onreadystatechange = function () {
        ...
        if (ajax.getResponseText() == '2') {
            Selector.$('ativo').focus();
            var mensagem = new DialogoMensagens("prompt", 120, 350, 150, "2", "Atenção!", "Esta conta já está cadastrado.", "OK", "", false, "nome");
            mensagem.Show();
            return;
        }
        ...
        if (ajax.getResponseText() == 0) {
            Selector.$('ativo').focus();
            var mensagem = new DialogoMensagens("prompt", 140, 350, 150, "1", "Erro!", "Erro ao gravar a conta. Se o erro persistir contate o suporte técnico.", "OK", "", false, "");
            mensagem.Show();
            return;
        } else {
