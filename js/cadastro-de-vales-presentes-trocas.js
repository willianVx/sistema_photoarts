checkSessao();
CheckPermissao(72, false, '', true);

window.onload = function(){

    Selector.$("legendaTela").innerHTML = "<div>Cadastro de Vales Presentes/Trocas</div>";
    carregarmenu();
    getDadosUsuario();

    setDataDeAte(Selector.$('de'), Selector.$('ate'));
    getLojas(Selector.$('galeria'), 'Todas as galrias', true);
...
function promptCadastro(codigo) {
    ...
    if(codigo <= 0){
        if(!CheckPermissao(73, true, 'Você não possui permissão para cadastrar um novo vale-presente', false)){
            return;
        }
    }
    ...
    if(codigo <= 0){
        getLojas(Selector.$('galeria2'), 'Todas', true);
        ...
        Selector.$('galeria2').focus();
        Selector.$('qtdParcelas').value = '1';
    }else{

        getLojas(Selector.$('galeria2'), 'Todas', false);
...
function Gravar(codigo) {
    ...
    if(!CheckPermissao(73, true, 'Você não possui permissão para editar um vale-presente', false)){
        return;
    }
...
function ExcluirValeAux(idValePresenteTroca) {
    ...
    if(!CheckPermissao(74, true, 'Você não possui permissão para excluir um vale-presente', false)){
        return;
    }
