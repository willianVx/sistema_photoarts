checkSessao();
CheckPermissao(105, false, '', true);

window.onload = function () {

    Selector.$("legendaTela").innerHTML = "<div>Repasses de Marchands Gerentes</div>";
    carregarmenu();
    getDadosUsuario();
    ...
    getMarchandsGerentes(Selector.$('marchand'), 'Todos', true, 1, 0);
...
function Pesquisar() {
    ...
    var ajax = new Ajax('POST', 'php/repasses-de-marchands-gerentes.php', true);
    ...
    ajax.ajax.onreadystatechange = function () {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            verConpag.setAttribute('onclick', 'window.location="cadastro-de-contas-a-pagar.html?source=repasses-de-marchands-gerentes&idConpag=' + json[i].idConpag + '"');
...
function PromptFecharRepasses(){
    ...
    if(!CheckPermissao(106, true, 'Você não possui permissão para fazer o fechamentos dos repasses', false)){
        return;
    }
...
function Imprimir_onClick() {
    ...
    if(!CheckPermissao(107, true, 'Você não possui permissão para imprimir o repasses de marchands gerentes', false)){
        return;
    }
...
function botExportar_onClick() {
    ...
    if(!CheckPermissao(108, true, 'Você não possui permissão para gerar o excel dos repasses dos marchands gerentes', false)){
        return;
    }
    ...
    var ajax = new Ajax('POST', 'php/repasses-de-marchands-gerentes.php', false);
