checkSessao();
CheckPermissao(97, false, '', true);

window.onload = function () {

    Selector.$("legendaTela").innerHTML = "<div>Repasses de Artistas</div>";
    carregarmenu();
    getDadosUsuario();
...
function Pesquisar() {
    ...
    if(Selector.$('artista').value > 0 && Selector.$('situacao').value == '1'){
    ...
    var ajax = new Ajax('POST', 'php/repasses-de-artistas.php', true);
    ...
    ajax.ajax.onreadystatechange = function () {
        ...
        if (ajax.getResponseText() === '0') {
            Selector.$('contador').innerHTML = "Nenhuma obra localizada";
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            verConpag.setAttribute('onclick', 'window.location="cadastro-de-contas-a-pagar.html?source=repasses-de-artistas&idConpag=' + json[i].idConpag + '"');
        ...
        Selector.$('contador').innerHTML = json.length + " parcela(s) localizada(s)";
        Selector.$('botPesquisar').value = 'Pesquisar';
    };
...
function PromptFecharRepasses(){
    ...
    if(!CheckPermissao(98, true, 'Você não possui permissão para fazer o fechamentos dos repasses', false)){
        return;
    }

    if(grid.getSelCount(0) <= 0){
        var mensagem = new DialogoMensagens("prompt", 120, 400, 150, "2", "Atenção!", "Favor selecionar uma ou mais obras para fechar", "OK", "", false, "");
        mensagem.Show();
        return;
    }
...
function Imprimir_onClick() {
    ...
    if(!CheckPermissao(99, true, 'Você não possui permissão para imprimir o repasses de artistas', false)){
        return;
    }
    ...
    window.open('impressao-de-relatorios.html?source=repasses-de-artistas');
}

function botExportar_onClick() {

    if(!CheckPermissao(100, true, 'Você não possui permissão para gerar o excel dos repasses dos artistas', false)){
        return;
    }
    ...
    var ajax = new Ajax('POST', 'php/repasses-de-artistas.php', false);
