            ...
            function Valida() {
                ...
                if (Selector.$('fileUpload').value.trim() == '') {
                    ...
                    //Selector.$('imgAdd').src = 'imagens/loading-2.gif';
                                    alert(Selector.$('fileUpload').value.trim());
...
$extensao = strtolower(end(explode('.', $_FILES['fileUpload']['nome'])));
...
if ($extensao != '') {
    ...
    if ($nalista == false) {
        $msgEx = '<font>Envie arquivos com as seguintes extensões: ' . $arquivotipo . '. até 50Mb.</font>';
    }
// Faz a verifica��o do tamanho do arquivo
    else if ($_UP['tamanho'] < $_FILES['fileUpload']['size']) {
        $msgTa = '<font color="#ff0000">O arquivo enviado é muito grande, envie arquivos de até 50Mb.</font>';
    }
// O arquivo passou em todas as verifica��es, hora de tentar mov�-lo para a pasta
        ...
        if ($_UP['renomeia'] == true) {
            ...
            removePontosArquivo($_FILES['fileUpload']['name']);
