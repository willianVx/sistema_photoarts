...
Date.GetDatePlus = function( Days, getHours) {
    ...
    return Date.FormatDDMMYYYY( d, getHours);
}
...
Table = function (id) {
    ...
    this.getContentSelectObjectRows = function (col) {
        //Objetos com ID
        var count = Array();
    ...
    this.getContentObjectTextComboRows = function (col) {
        //Obtém os textos das combos dos objetos dentro da célula
        var count = Array();

        for (var i = 0; i < this.getRowCount(); i++) {
            count.push(this.getCellObject(i, col).options[this.getCellObject(i, col).selectedIndex].text);
        }
    ...
    this.getContentObjectValueRows = function (col) {
        //Obtém os valores dos objetos dentro da célula
        var count = Array();
    ...
    this.getContentObjectNameRows = function (col) {
        ...
        for (var i = 0; i < this.getRowCount(); i++) {
            count.push(this.getCellObject(i, col).name);
        }

        return count;
    };

    this.getContentSelectObjectNameRows = function (col) {
        var count = Array();
    ...
    this.getContentSelectObjectId = function (col) {
        var count = '';

        for (var i = 0; i < this.getRowCount(); i++) {
            if (this.getCellObject(i, 0).checked) {
                if (count === '')
                    count = this.getCellObject(i, col).id;
                else
                    count += '#' + this.getCellObject(i, col).id;
            }
    ...
    this.getContentObjectId = function (col) {
        var count = '';
    ...
    this.getContentObjectValueMoneyRows = function (col) {
        ...
        for (var i = 0; i < this.getRowCount(); i++) {
            if (this.getCellObject(i, col).value == '') {
                count.push('0');
            }
            else {
    ...
    this.getContentSelectRows = function (col) {
        var count = Array();
    ...
    this.getContentObjectRowsTitle = function (col) {
        var count = Array();

        for (var i = 0; i < this.getRowCount(); i++) {

            if (this.getCellObject(i, col).title.toString().indexOf('/') == -1) {
                count.push(this.getCellObject(i, col).title);
            } else {
                count.push(this.getCellObject(i, col).title.toString().split('/')[this.getCellObject(i, col).title.toString().split('/').length - 1]);
            }
        }
    ...
    this.getContentObjectRowsSrcFile = function (col) {
        ...
        for (var i = 0; i < this.getRowCount(); i++) {
            ...
            if (this.getCellObject(i, col).src.toString().indexOf('/') == -1) {
                count.push(this.getCellObject(i, col).src);
            } else {
                count.push(this.getCellObject(i, col).src.toString().split('/')[this.getCellObject(i, col).src.toString().split('/').length - 1]);
            }
        }
...
function DialogUploadNovo(div, nomeImg, path, funcao, tema, imagePath, tiposdearquivos) {
    ...
    iframe.src = caixaDialogo.imagePath + 'upFiles.php?nome=' + nomeImg + "&path=" + path + "&funcao=" + funcao + "&tema=" + tema + "&arquivotipo=" + tiposdearquivos;
    dialog.Show();
